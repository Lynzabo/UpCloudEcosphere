...
AddStructSendAddrServlet::AddStructSendAddrServlet()
{

}

void AddStructSendAddrServlet::doExec(DRequestContext *request, DResponseContext *response)
{
    ...
    //查询是否节点已存在
    if((CSStructDao::dirRSendAddr(taskUuid,nodes)).size() > 0) {
    ...
    //处理业务
    try{
        CSStructDao::addRSendAddr(taskUuid,nodes,groupDes);
    }
