...
DelStructTaskAddrRunnable::DelStructTaskAddrRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
...
void DelStructTaskAddrRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Struct_delTaskNode);
...
void DelStructTaskAddrRunnable::setAddrInfo(const QtJson::JsonArray &json, const QString &taskuuid)
{
    m_taskUuid = taskuuid;
    m_uuids = json;
}
