...
DelUnstructTaskRunnable::DelUnstructTaskRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
...
void DelUnstructTaskRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_delTask);
...
void DelUnstructTaskRunnable::setTaskUuids(const JsonArray &json)
{
   m_uuids = json;
}
