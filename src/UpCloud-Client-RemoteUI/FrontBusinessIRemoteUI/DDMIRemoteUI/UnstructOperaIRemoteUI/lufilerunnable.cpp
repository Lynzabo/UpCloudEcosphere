...
ListUnstructFileRunnable::ListUnstructFileRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
    //注册VCsRUnstructSendFiles元数据
    qRegisterMetaType<VCsRUnstructSendFiles>("VCsRUnstructSendFiles");
    qRegisterMetaType<QList<VCsRUnstructSendFiles> >("QList<VCsRUnstructSendFiles>");
}



void ListUnstructFileRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    bool ok;
    JsonObject json;
    QList<VCsRUnstructSendFiles> taskList;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_listFile);
...
//        m_taskUuid = resultObj.value("resultStr").toString();
        ...
        ok = UnstructUtil::taskVAddrFileEntityFromjsonStr(taskList,resultStr);
        if (!ok)
        {
paramsError:
            qDebug("解析返回数据失败");
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<VCsRUnstructSendFiles>,taskList)
                              );
...
void ListUnstructFileRunnable::setTaskUUID(const QString &taskuuid)
{
    m_taskUuid = taskuuid;
}

void ListUnstructFileRunnable::setFilterPage(Page *page)
{
    m_page = page;
}
