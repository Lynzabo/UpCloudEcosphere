...
#include <QVBoxLayout>
#include <QHBoxLayout>
DownView::DownView(QWidget* parent)
{
...
void DownView::setProcess(float per,const QString &str,const QString &time)
{
    setValue((int)per);
...
void DownView::updateInfo(quint64 haveDown, quint64 total)
{
    ...
    double speed = (haveDown - m_downBytes) * 1000.0 / 1024/usedTime;
    ...
    m_downBytes = haveDown;
    m_curTime = curTime;
}

void DownView::setEnd()
{

}

void DownView::setStart()
{

}

void DownView::setError()
{

}

void DownView::setFileName(const QString& name)
{
    m_fileName = name;
    m_lab_name->setText(name);
}

void DownView::initView()
{
...
void DownView::pauseDown()
{
    m_btn_opt->setIcon(QIcon(":/START"));
    m_lab_time->setText("--:--:--");
}

void DownView::on_end(bool flg)
{
    Q_UNUSED(flg)
//    this->hide();
    emit evt_end(this);
...
void DownView::on_start(bool flg)
{
    Q_UNUSED(flg)
    if(m_flg) {
        m_btn_opt->setIcon(QIcon(":/START"));
...
QString DownView::fileSizeUnit(quint64 &size)
{
    if(size/1024 == 0) {
        return "B";
    }
...
QString DownView::speedUnit(double &size)
{
    if(size/1024 < 1) {
        return "B/S";
    }
...
QTime DownView::curTime() const
{
    return m_curTime;
}

void DownView::setCurTime(const QTime &curTime)
{
    m_curTime = curTime;
}

quint64 DownView::downBytes() const
{
    return m_downBytes;
}

void DownView::setDownBytes(const quint64 &haveDownBytes)
{
    m_downBytes = haveDownBytes;
}

quint32 DownView::dec() const
{
    return m_dec;
}

void DownView::setDec(const quint32 &dec)
{
    m_dec = dec;
}

QString DownView::fileName() const
{
    return m_fileName;
}
