#include "nsdownexector.h"
using namespace Environment::Task_Status::Noder::Stru;
using namespace Environment::Datagram::Stru_Data_Trans;

QMutex NoderStructDownExector::mutex;
NoderStructDownExector::NoderStructDownExector()
{
    //初始化任务集下载出错文件
...
void NoderStructDownExector::doExecute(QMap<QTcpSocket *, quint64> &m_socks, PushContext *context)
{
    mutex.lock();
    qDebug() << trs("Struct download exector ready to work");
    try
    {
        ...
        {
//            qDebug() << trs("Struct download exector exit,task is null");
            mutex.unlock();
            return;
        }
        //从配置文件读取网络连接配置
        ...
        for(NsRStructTask task : usableStructTasks)
        {
            ...
            connect(ftpPool,&FTPClientPool::subFileEnd,this,&NoderStructDownExector::on_TransSubFileEnd,Qt::DirectConnection);
            connect(ftpPool, &FTPClientPool::end, this, &NoderStructDownExector::on_TransEnd,Qt::DirectConnection);
            connect(ftpPool,&FTPClientPool::error,this,&NoderStructDownExector::on_TransError,Qt::DirectConnection);
            //构建HTTP下载器,存放到任务HTTP下载器集
            HttpCientPool* httpPool = new HttpCientPool(taskUUID);
            connect(httpPool,&HttpCientPool::subFileEnd,this,&NoderStructDownExector::on_TransSubFileEnd,Qt::DirectConnection);
            connect(httpPool, &HttpCientPool::end, this, &NoderStructDownExector::on_TransEnd,Qt::DirectConnection);
            connect(httpPool,&HttpCientPool::error,this,&NoderStructDownExector::on_TransError,Qt::DirectConnection);
            ...
            for(NsRStructSendFiles sendFile : sendFiles)
            {
                ...
                NsStructDao::updateRSendFilesStateByUrlAbst(taskUUID, fileUrl, destAbst, SendFiles_State::DOWNING);
    ...
    qDebug() << trs("Struct download exector download complete");
    mutex.unlock();
}

void NoderStructDownExector::doSetArgs(QMap<QTcpSocket *, quint64> &socks)
{
    m_socks = socks;
}

void NoderStructDownExector::doExecute()
{
    PushContext *context = new PushContext;
    this->doExecute(m_socks, context);
}

void NoderStructDownExector::on_RemoteHostClosed(const QString &clientUUID)
{
    //TODO修改任务状态
}

void NoderStructDownExector::on_TransEnd(const QString &taskID)
{
    endMutex->lock();
    ...
    ////拼装反馈下载状态报文
    ReqStructFilesDownloadStatus* reqEntity = new ReqStructFilesDownloadStatus;
    ...
    if(taskErrorFiles->size() > 0)
    {
        ...
        NsStructDao::updateRTaskState(taskUUID,Task_State::DOWNERROR);
        //发送出错回执报文
        goto sendDatagram;
    }
    else
    {
        ...
        NsStructDao::updateRTaskState(taskUUID,Task_State::DOWNSUCCESS);
        //发送下载成功回执报文
        goto sendDatagram;
    }
...
void NoderStructDownExector::on_TransSubFileEnd(const QString &taskID, const QString &fileUrl, const QString &destAbst)
{
    sfeMutex->lock();
    QString taskUUID = taskID;
    try
    {
        NsStructDao::updateRSendFilesStateByUrlAbst(taskUUID, fileUrl, destAbst, SendFiles_State::DOWNSUCCESS);
    }
    catch(DBException &excep)
    {
        qDebug() << trs("Error:%1,%2,%3").arg(excep.getType()).arg(excep.getTypeDesc()).arg(excep.getText());
...
void NoderStructDownExector::on_TransError(const QString &taskID, const QString &src, const QString &errMsg)
{
    errMutex->lock();
    QString taskUUID = taskID;
    ...
    try
    {
        NsStructDao::updateRSendFilesState(taskUUID, fileUrl, SendFiles_State::DOWNERROR,errMsg);
    }
    catch(DBException &excep)
    {
        qDebug() << trs("Error:%1,%2,%3").arg(excep.getType()).arg(excep.getTypeDesc()).arg(excep.getText());
