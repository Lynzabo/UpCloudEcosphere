...
UpdateUnstructTaskAddrServlet::UpdateUnstructTaskAddrServlet()
{

}

void UpdateUnstructTaskAddrServlet::doExec(DRequestContext *request, DResponseContext *response)
{
    ...
    //处理业务
    try{
        CSUnstructDao::updateRTaskSendStatusByTaskUUIDAndClientUUID(taskUuid,nodes,state);
    }
