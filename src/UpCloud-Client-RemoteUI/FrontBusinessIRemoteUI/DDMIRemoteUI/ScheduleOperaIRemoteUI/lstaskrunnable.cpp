...
ListCsRScheduleTaskRunnable::ListCsRScheduleTaskRunnable(QObject* object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
    qRegisterMetaType<CsRScheduleTask>("CsRScheduleTask");
    qRegisterMetaType<QList<CsRScheduleTask> >("QList<CsRScheduleTask>");
}

void ListCsRScheduleTaskRunnable::setScheduleDirinfo(const QVariantMap &params)
{
    m_params = params;
}

void ListCsRScheduleTaskRunnable::run()
{
    bool ok = false;
    quint8 rState;
    QString result;result.clear();
    QList<CsRScheduleTask> taskList;taskList.clear();
    //准备参数 序列化
    ...
    sender->setExecType(Control_Module::Schedule_listTask);
    ...
    //操作成功
        ...
        ok = ScheduleUtil::taskListEntinyFromJson(taskList,result);
        if (!ok)
        {
            qDebug("解析返回数据失败");
            m_cbMsgID = CallbackStatus::Schedule::_6;
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<CsRScheduleTask>,taskList)
                              );
