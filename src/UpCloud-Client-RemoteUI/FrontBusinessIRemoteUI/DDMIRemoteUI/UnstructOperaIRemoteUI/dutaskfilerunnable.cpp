...
DelUnstructTaskFileRunnable::DelUnstructTaskFileRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
...
void DelUnstructTaskFileRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_delTaskFile);
...
void DelUnstructTaskFileRunnable::setFileUuids(const QString &taskuuid, const QtJson::JsonArray &uuids)
{
    m_taskUuid = taskuuid;
    m_uuids = uuids;
}
