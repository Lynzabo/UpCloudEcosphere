...
namespace QtJson {
    ...
    QVariant parse(const QString &json) {
        bool success = true;
        QVariant var = parse(json, success);
        if(var.isNull())
            throw JsonException(QObject::tr("Failed to parse the json string, Strings of length 0"));
        else if(!success)
            throw JsonException(QObject::tr("Failed to parse the json string, check the string format:").append(json));
        return var;
    }
    ...
    QByteArray serialize(const QVariant &data) {
        bool success = true;
        QByteArray array = serialize(data, success);
        if(array.isNull() || !strcmp(array.toStdString().data(),"null"))
            throw JsonException(QObject::tr("Failed to serialize the data to json string, Strings of length 0"));
        else if(!success)
            throw JsonException(QObject::tr("Failed to serialize the data to json string"));
        return array;
    }
