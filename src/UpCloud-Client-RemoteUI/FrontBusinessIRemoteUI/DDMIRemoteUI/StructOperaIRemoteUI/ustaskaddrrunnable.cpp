...
UpdateStructTaskAddrRunnable::UpdateStructTaskAddrRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
...
void UpdateStructTaskAddrRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Struct_updateTaskAddr);
...
void UpdateStructTaskAddrRunnable::setUpdateInfo(const QStringList &uuids, const QString &taskuuid, const quint8 &state)
{
    m_uuids = uuids;
    m_taskUuid = taskuuid;
    m_state = state;
