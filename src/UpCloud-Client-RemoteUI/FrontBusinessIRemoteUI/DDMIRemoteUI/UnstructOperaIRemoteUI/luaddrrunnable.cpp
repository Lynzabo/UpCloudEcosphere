...
ListUnstructAddrRunnable::ListUnstructAddrRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
    //注册CsRUnstructSendFiles元数据
    qRegisterMetaType<VCsRUnstructSendAddr>("VCsRUnstructSendAddr");
    qRegisterMetaType<QList<VCsRUnstructSendAddr> >("QList<VCsRUnstructSendAddr>");
}

void ListUnstructAddrRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    QList<VCsRUnstructSendAddr> taskList;
    bool ok;
...
//        m_taskUuid = resultObj.value("resultStr").toString();
        ...
        ok = UnstructUtil::taskVAddrEntityFromjsonStr(taskList,resultStr);
        if (!ok)
        {
paramsError:
            qDebug() << "解析返回数据失败";
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<VCsRUnstructSendAddr>,taskList)
                              );
...
void ListUnstructAddrRunnable::setTaskUUID(const QString &taskuuid)
{
    m_taskUuid = taskuuid;
}

void ListUnstructAddrRunnable::setFilterPage(Page *page)
{
    m_page = page;
}
