...
UpdateScheduleTaskRunnable::UpdateScheduleTaskRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
...
void UpdateScheduleTaskRunnable::setTaskList(const QList<CsRScheduleTask> &list)
{
    m_taskList = list;
}

void UpdateScheduleTaskRunnable::run()
{
    bool ok = false;
    quint8 rState;
   ...
   ScheduleUtil::taskListEntinyToJson(params,m_taskList);
    //准备消息发送端
    ...
    sender->setExecType(Control_Module::Schedule_updateTask);
