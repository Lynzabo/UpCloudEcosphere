...
ListVCsAScSendFilesRunnable::ListVCsAScSendFilesRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
    qRegisterMetaType<VCsAScSendFiles>("VCsAScSendFiles");
    qRegisterMetaType<QList<VCsAScSendFiles> >("QList<VCsAScSendFiles>");
}

void ListVCsAScSendFilesRunnable::setTaskUuid(const QString &taskUuid)
{
 m_taskUuid = taskUuid;
}

void ListVCsAScSendFilesRunnable::run()
{
    bool ok = false;
    quint8 rState;
    ...
    sender->setExecType(Control_Module::Schedule_listFiles);
    ...
    //操作成功
        ...
        ok = ScheduleUtil::vFileListEntinyFromJson(vFileList,result);
        if (!ok)
        {
            qDebug("解析返回数据失败");
            m_cbMsgID = CallbackStatus::Schedule::_6;
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<VCsAScSendFiles>,vFileList)
                              );
