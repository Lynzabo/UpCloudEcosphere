...
DelScheduleTaskAddrRunnable::DelScheduleTaskAddrRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
...
void DelScheduleTaskAddrRunnable::setAddrInfo(const QString &taskUuid, const QStringList &clientUuids)
{
    m_taskUuid = taskUuid;
    m_clientUuids = clientUuids;
}

void DelScheduleTaskAddrRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    //准备参数 序列化
    JsonObject json;
    QString nodesStr = QtJson::serialize(m_clientUuids);
    ...
    sender->setExecType(Control_Module::Schedule_delAddr);
