#include "lusendstatusrunnable.h"
...
ListUnstructSendStatusRunnable::ListUnstructSendStatusRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
...
void ListUnstructSendStatusRunnable::run()
{
    bool ok;
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_listStatus);
    ...
    //TODO 需要解析结果集
        ...
        ok = UnstructUtil::taskSendStatusEntityFromjsonStr(statusList,statusStr);
        if (!ok)
        {
paramsError:
            m_cbMsgId = CallbackStatus::Unstruct::_4;
    ...
    if(m_fileName.isEmpty()) {
        QMetaObject::invokeMethod(m_object,"on_itemtaskCallbackNode",Qt::QueuedConnection,
                                  ...
                                  Q_ARG(QList<CsRUnstructSendStatus>,statusList),
...
void ListUnstructSendStatusRunnable::setNodeUuid(const QString &taskUuid,const QString &clientUuid)
{
    m_clientUuid = clientUuid;
    m_taskUuid = taskUuid;
}

void ListUnstructSendStatusRunnable::setFileName(const QString &taskUuid, const QString &fileName)
{
    m_fileName = fileName;
    m_taskUuid = taskUuid;
}

void ListUnstructSendStatusRunnable::setFilterPage(Page *page)
{
    m_page = page;
}
