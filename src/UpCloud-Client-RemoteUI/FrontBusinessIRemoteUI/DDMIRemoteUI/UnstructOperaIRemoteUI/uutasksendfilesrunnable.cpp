#include "uutasksendfilesrunnable.h"
...
UpdateUnstructTaskSendFilesRunnable::UpdateUnstructTaskSendFilesRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
...
void UpdateUnstructTaskSendFilesRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_updateTaskFile);
...
void UpdateUnstructTaskSendFilesRunnable::setUpdateState(const QStringList &Uuids, const QString &taskuuid, const quint8 state)
{
    m_uuids = Uuids;
    m_state = state;
    m_taskUuid = taskuuid;
