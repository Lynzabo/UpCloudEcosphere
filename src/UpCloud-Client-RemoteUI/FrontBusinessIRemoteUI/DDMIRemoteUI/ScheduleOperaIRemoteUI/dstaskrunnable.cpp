...
DelScheduleTaskRunnable::DelScheduleTaskRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
...
void DelScheduleTaskRunnable::setTaskUuid(const QStringList &uuids)
{
    m_uuids = uuids;
}

void DelScheduleTaskRunnable::run()
{
    bool ok = false;
    quint8 rState;
    ...
    //准备参数 序列化
    QString params = QtJson::serialize(m_uuids);
    //准备消息发送端
    ...
    sender->setExecType(Control_Module::Schedule_delTask);
