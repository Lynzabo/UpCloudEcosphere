#include "lcextendinforunnable.h"
...
ListClientExtendInfoRunnable::ListClientExtendInfoRunnable(QObject *object):m_object(object)
{
   m_cbMsg.clear();
   m_cbMsgId = 0;
   qRegisterMetaType<CsClientExtendInfo>("CsClientExtendInfo");
   qRegisterMetaType<QList<CsClientExtendInfo> >("QList<CsClientExtendInfo>");
}

void ListClientExtendInfoRunnable::setExtendInfo(const QString &clientUuid)
{
    m_clientUuid = clientUuid;
    m_clientUuid.append("%");
}

void ListClientExtendInfoRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    QList<CsClientExtendInfo> taskList;

    QString params = QtJson::serialize(m_clientUuid);
    bool ok;
    //准备消息发送端
    ...
    //准备数据
    sender->setExecType(Control_Module::Group_listExtendInfo);
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<CsClientExtendInfo>,taskList)
                              );
