...
UpdateUnstructTaskAddrRunnable::UpdateUnstructTaskAddrRunnable(QObject *object):m_object(object)
{
    m_cbMsgId = 0;
    m_cbMsg.clear();
...
void UpdateUnstructTaskAddrRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Unstruct_updateTaskNode);
...
void UpdateUnstructTaskAddrRunnable::setUpdateState(const QStringList &clientUuids, const QString &taskuuid, const quint8 state)
{
    m_uuids = clientUuids;
    m_taskUuid = taskuuid;
    m_state = state;
