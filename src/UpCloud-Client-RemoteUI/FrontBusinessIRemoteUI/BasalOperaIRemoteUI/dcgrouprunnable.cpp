...
DelClientGroupRunnable::DelClientGroupRunnable(QObject* object):m_object(object)
{
   m_cbMsg.clear();
   m_cbMsgId = 0;
}
...
void DelClientGroupRunnable::setGroupInfo(const QVariantMap &params)
{
    m_params = params;
}

void DelClientGroupRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Group_delClientGroup);
