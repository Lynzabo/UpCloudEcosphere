...
ListStructFileRunnable::ListStructFileRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
    qRegisterMetaType<VCsRStructSendFiles>("VCsRStructSendFiles");
    qRegisterMetaType<QList<VCsRStructSendFiles> >("QList<VCsRStructSendFiles>");
}

void ListStructFileRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    QList<VCsRStructSendFiles> taskList;
    bool ok;
    JsonObject json;
    ...
    //准备数据
    sender->setExecType(Control_Module::Struct_listFile);
...
paramsError:
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<VCsRStructSendFiles>,taskList)
                              );
...
void ListStructFileRunnable::setListCondition(const QString &taskuuid)
{
    m_taskUuid = taskuuid;
}

void ListStructFileRunnable::setFilterPage(Page *page)
{
    m_page = page;
}
