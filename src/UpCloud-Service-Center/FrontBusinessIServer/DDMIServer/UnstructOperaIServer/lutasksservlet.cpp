...
ListUnstructTasksServlet::ListUnstructTasksServlet()
{
...
void ListUnstructTasksServlet::doInit()
{
    page = new Page;
    //设置当前页码
...
void ListUnstructTasksServlet::doExec(DRequestContext *request, DResponseContext *response)
{
    ...
    //获取分页列表数据
    try{
        taskList = CSUnstructDao::pageTask(paramsObj,this->page);
    }
    ...
    //组装分页数据
    UnstructUtil::taskEntityTojsonStr(resultStr,taskList);
