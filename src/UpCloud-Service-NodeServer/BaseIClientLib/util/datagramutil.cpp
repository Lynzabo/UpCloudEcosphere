...
void DatagramUtil::init()
{
    QString pubKeyFile;
    QString priKeyFile;
    pubKeyFile= NSAbstPath::getInstance().cryAbst()+"/node_pub.key";
    priKeyFile= NSAbstPath::getInstance().cryAbst()+"/node_pri.key";
//    pubKeyFile= "C:/UpCloud-package/NoderServer/data/cry""/node_pub.key";
//    priKeyFile= "C:/UpCloud-package/NoderServer/data/cry""/node_pri.key";

    QFile pubFile(pubKeyFile);
    if(pubFile.open(QFile::ReadOnly| QFile::Text) == true)
    {
        m_pubKey = QString(pubFile.readAll());
        pubFile.close();
        qDebug() << "m_pubKey" << m_pubKey;
    }
    else
    {
        m_pubKey = "";
    ...
    if(priFile.open(QFile::ReadOnly| QFile::Text) == true)
    {
        m_priKey = QString(priFile.readAll());
        priFile.close();
        qDebug() << "m_priKey" << m_priKey;
    }
    else
    {
        m_priKey = "";
...
void DatagramUtil::packDatagram(MessageInfo *dataIn, QByteArray &datagram)
{
    ...
    enDecrypt.encryption(m_pubKey,condram,cipherText);
