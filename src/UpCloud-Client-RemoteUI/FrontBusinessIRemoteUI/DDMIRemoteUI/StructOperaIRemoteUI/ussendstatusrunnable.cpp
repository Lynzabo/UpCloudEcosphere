#include "ussendstatusrunnable.h"
#include "avro.h"
...
UpdateStructSendStatusRunnable::UpdateStructSendStatusRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
}

void UpdateStructSendStatusRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Struct_updateStatus);
...
void UpdateStructSendStatusRunnable::setUpdateState(const QStringList &uuids, const QString &taskUuid, const quint8 state)
{
    m_uuids = uuids;
    m_taskUuid = taskUuid;
    m_state = state;
