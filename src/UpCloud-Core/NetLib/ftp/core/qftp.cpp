...
public:
    ...
    void setBytesDone(qint64 bytes);                    //zhj
...
void QFtpDTP::connectToHost(const QString & host, quint16 port)
{
    ...
    qDebug()<<"QFtpDTP Passive state socket";
...
//    QString bufferStr = QString::fromLatin1(buffer).trimmed();
    ...
    QString bufferStr = QString(buffer).trimmed();
...
#endif
            if (parseDir(line, QLatin1String(""), &i)) {                   //函数有问题
                emit listInfo(i);
            } else {
...
void QFtpPI::abort()
{
    ...
    if (currentCmd.startsWith(QLatin1String("STOR "))
            ...
            || currentCmd.startsWith(QLatin1String("RETR "))) // zhj add APPE RETR
        dtp.abortConnection();
}
...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
    ...
    {
        //close socket
        qDebug()<<"reply 221 Goodbye._____";
        ...
        if (addrPortPattern.indexIn(replyText) == -1) {
            ...
            dtp.connectToHost(host, port);              //连接到服务器
        }
...
#endif
    state = Waiting;
    commandSocket.write(currentCmd.toStdString().c_str()/*.toLatin1()*/);
    return true;
}
...
void QFtp::size(const QString &fileUrl)
{
    rawCommand(QLatin1String("SIZE ") + fileUrl);
}
...
int QFtp::conPut(QIODevice *dev, const QString &file, TransferType type)
{
    ...
    if (type == Binary)
        ...
        cmds << QLatin1String("TYPE A\r\n");
    cmds << QLatin1String("SIZE ") + dev->size() + QLatin1String("\r\n");
    cmds << QLatin1String(d->transferMode == Passive ? "PASV\r\n" : "PORT\r\n");
    if (!dev->isSequential())
        cmds << QLatin1String("ALLO ") + QString::number(dev->size()) + QLatin1String("\r\n");
