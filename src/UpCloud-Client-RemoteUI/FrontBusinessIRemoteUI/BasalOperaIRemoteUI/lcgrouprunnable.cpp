...
ListClientGroupRunnable::ListClientGroupRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
...
void ListClientGroupRunnable::setGroupInfo(const QStringList &list, const quint32 page)
{
   m_supUuids = list;
   m_page = page;
   m_supUuids.append(QString::number(m_page));
}


void ListClientGroupRunnable::run()
{
    quint8 rState;
    QString result;result.clear();
    QList<CsClientGroup> taskList;
    QString params = QtJson::serialize(m_supUuids);
    bool ok;
    //准备消息发送端
    ...
    //准备数据
    sender->setExecType(Control_Module::Group_listClientGroup);
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<CsClientGroup>,taskList)
                              );
