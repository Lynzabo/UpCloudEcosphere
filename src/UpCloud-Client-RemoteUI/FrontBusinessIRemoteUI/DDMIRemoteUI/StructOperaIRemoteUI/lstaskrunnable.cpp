...
ListStructTaskRunnable::ListStructTaskRunnable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgId = 0;
...
void ListStructTaskRunnable::run()
{
    quint8 rState;
    ...
    //准备数据
    sender->setExecType(Control_Module::Struct_listTask);
    ...
    QMetaObject::invokeMethod(m_object,"on_itemtaskCallback",Qt::QueuedConnection,
                              ...
                              Q_ARG(QList<CsRStructTask>,taskList)
                              );

     sender->destory();
}

void ListStructTaskRunnable::setTaskInfo(const QVariantMap &map)
{
    m_map = map;
}

void ListStructTaskRunnable::setFilterPage(Page *_page)
{
    m_page = _page;
}
