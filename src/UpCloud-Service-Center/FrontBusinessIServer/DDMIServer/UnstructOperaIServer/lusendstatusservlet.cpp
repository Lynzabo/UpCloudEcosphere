...
ListUnstructSendStatusServlet::ListUnstructSendStatusServlet()
{
...
void ListUnstructSendStatusServlet::doInit()
{
    this->page = new Page;
...
void ListUnstructSendStatusServlet::doExec(DRequestContext *request, DResponseContext *response)
{
    ...
    try{
        if(fileName.isEmpty()) {
            statusList = CSUnstructDao::pageRSendStatusByTaskUuidAndClientUuid(taskUuid,clientUuid,nameList,this->page);
        }
        else {
            statusList =  CSUnstructDao::pageRSendStatusByTaskUuidAndSendFileUUid(taskUuid,fileName,nameList,this->page);
        }
    }
    ...
    //返回查询结果
    UnstructUtil::taskSendStatusEntityTojsonStr(statusStr,statusList);
