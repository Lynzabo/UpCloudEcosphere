...
DelScheduleTaskFileRunnable::DelScheduleTaskFileRunnable(QObject* object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
}

void DelScheduleTaskFileRunnable::setFilesInfo(const QStringList &uuids)
{
    m_uuids = uuids;
}

void DelScheduleTaskFileRunnable::run()
{
    quint8 rState;
    ...
    sender->setExecType(Control_Module::Schedule_delFiles);
