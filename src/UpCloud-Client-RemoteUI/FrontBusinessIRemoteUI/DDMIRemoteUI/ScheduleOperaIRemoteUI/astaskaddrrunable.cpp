...
AddScheduleTaskAddrRunable::AddScheduleTaskAddrRunable(QObject *object):m_object(object)
{
    m_cbMsg.clear();
    m_cbMsgID = 0;
...
void AddScheduleTaskAddrRunable::setAddrInfo(const QString &taskUuid,const QStringList &nodes, const QStringList &groupDes)
{
    m_taskUuid = taskUuid;
    m_nodes = nodes;
    m_groupDes = groupDes;
...
void AddScheduleTaskAddrRunable::run()
{
    quint8 rState;
    QString result;result.clear();
    //准备参数 序列化
    JsonObject json;
    QString nodesStr = QtJson::serialize(m_nodes);
    QString groupStr = QtJson::serialize(m_groupDes);
    json.insert("taskUuid",m_taskUuid);
    json.insert("nodes",nodesStr);
    json.insert("description",groupStr);
    ...
    sender->setExecType(Control_Module::Schedule_addAddr);
